...
raylib-Computer-Graphics-Sandbox is an in-development sandbox application for 3D computer graphics developed for personal learning written in C using [raylib](https://www.raylib.com/index.html), its [raygui library](https://github.com/raysan5/raygui), and [cJSON](https://github.com/DaveGamble/cJSON).
...
- The app updates both its UI *and* its drawing logic on each frame. If you're at a low framerate (e.g. 1-10 frames per second) and the app seems unresponsive, try repeatedly clicking a higher position on the framerate slider (unless you want it that way, of course).
## Known Issues
- Vertices in certain positions (off screen or otherwise) will be rendered when they shouldn't, often in strange ways.
